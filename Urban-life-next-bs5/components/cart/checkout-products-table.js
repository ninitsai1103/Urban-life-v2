...
export default function CheckoutProductsTable() {
  ...
  // 監聽全選狀態變化，同步更新購物車項目的 checked 屬性
  // useEffect(() => {
  //   if (checkAll) {
  //     // 如果全選按鈕被勾選，則將所有購物車項目的 checked 屬性設置為 true
  //     const nextProductsChecked = items.map((v) => {
  //       return { ...v, checked: true }
  //     })
  //     setItems(nextProductsChecked)
  //   } else if (!checkAll && !state) {
  //     // 如果全選按鈕被取消勾選，且不是因為 state 變量的改變，則將所有購物車項目的 checked 屬性設置為 false
  //     // const nextProductsChecked = items.map((v) => {
